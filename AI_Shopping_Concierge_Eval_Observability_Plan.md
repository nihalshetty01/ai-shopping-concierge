...
24 hand-crafted examples across all 6 use cases plus 10 deliberate edge/adversarial cases — expanded after the v2 ranking logic (category-specific scoring) and the parse_constraints ambiguous-signal design (Rule 2a) added new failure modes to guard against. Each has a defined input, the behavior expected of the agent, and a concrete pass/fail check — not a vibe check.
...
| G23 | Ambiguous signal (budget) | "I'm looking for cheap mobile headphones" | `parse_constraints` returns `budget=None` with `ambiguous_signals: [{"type": "budget", "term": "cheap"}]`; per Rule 2a, the orchestrator's one clarifying question must target pinning down what "cheap" means in ₹ terms — never silently assumes a number | Response asks a specific budget-range question referencing the user's own word (e.g. "when you say cheap, what price range works for you?"); never recommends using a guessed budget |
| G24 | Ambiguous signal (priority) | "want something with great sound, price isn't a huge deal" | `parse_constraints` flags "great sound" as an ambiguous priority signal rather than silently setting priority="sound_quality"; orchestrator asks a targeted question to confirm before treating it as explicit | Response confirms sound_quality as the priority with the user rather than assuming it from the phrase alone |
...
### 3.1 User feedback events (explicit, self-reported)

Two distinct feedback signals are captured, both in `feedback_log.jsonl` with a `feedback_type` field distinguishing them — they measure different things and neither substitutes for the other:

**Product-level** (`feedback_type: "product"`) — was this *specific recommended product* a good pick? Feeds directly into evaluating `catalog_search`'s ranking quality.
...
**Response-level** (`feedback_type: "response"`) — was the *orchestrator's overall answer* — its reasoning, its explanation, its clarifying question — actually helpful? This is the signal more directly tied to the PRD's core value prop (decision confidence), since a user can find individual products unappealing while still finding the reasoning genuinely useful, or vice versa — the two are not the same thing and conflating them would hide real information.
...
```json
{
  ...
  "turn_index": 2,
  "rating": "up",
  "had_clarifying_question": true,
  "num_products_shown": 3
}
```

Both are the only *truthful* outcome signals this prototype can capture, since there is no checkout flow to confirm an actual purchase (see PRD Section 13, "No checkout"). Neither is a purchase-confirmation signal and neither should be reported or discussed as one — see PRD Section 18 for why that distinction matters and what it would take to close the gap in production.
...
| Product-level thumbs-up rate (overall and by category/priority) | Direct signal of ranking quality — did `catalog_search` surface the right items | Track as baseline; no target yet without real usage data |
| Response-level thumbs-up rate, split by whether a clarifying question was involved | Direct signal of conversational quality — tests whether asking a clarifying question correlates with a *more* or *less* helpful response, distinct from whether any listed product itself was liked | Track as baseline; no target yet without real usage data |
