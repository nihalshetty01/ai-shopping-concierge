...
1a. `parse_constraints` output is a fast structured pre-fill, not the final word — the orchestrator always cross-checks it against its own reading of the raw user message, since the tool is deterministic/regex-based and can miss things a language model catches naturally (e.g., "sixty thousand" instead of "60k")
2. Ask exactly ONE clarifying question if information is missing
2a. If `parse_constraints` returns an `ambiguous_signals` entry (e.g., the user said "cheap," "great sound," or "nothing fancy" without a number or explicit priority), the one clarifying question must target resolving that specific ambiguity directly — never silently assign a number or priority to subjective language. This mirrors Rule 7 one layer earlier: guessing what "cheap" means is inventing a constraint the user didn't give
...
- **Data provenance note (caught during build):** PassMark's public coverage of newer mobile chipsets is incomplete — when building the catalog, several phone chipsets initially came back with *extrapolated* rather than confirmed benchmark scores. This was caught during manual review and fixed by swapping those catalog entries for real phones with confirmed PassMark scores, rather than keeping interpolated numbers. This is the same "never invent a spec" principle from Rule 7 applied to the catalog's own construction, not just the agent's live responses — worth citing directly as an example of catching a grounding failure before it shipped.
...
| **Issue** | The orchestrator has been observed re-ordering `catalog_search`'s output for presentation (e.g. leading with an in-budget option over the tool's higher-scoring pick) — this isn't currently forbidden or permitted anywhere in the spec | Decide explicitly: is presentation order allowed to diverge from `final_score` order, and if so under what conditions (e.g. budget clarity) — then state it as a rule and add a golden example |
| **Issue** | Some specs (e.g. GPU model) exist only inside a product's free-text `name` field, not as a dedicated catalog field — `verify_grounding`'s planned deterministic field-matching (Section 10) cannot validate a claim like "RTX 3050" against a field that doesn't exist | Must be resolved as part of building `verify_grounding`: either add structured fields for every spec the agent might cite, or explicitly scope grounding checks to structured fields only and treat name-string claims as a known, documented limitation |
...
## 18. Feedback Loop & Data Flywheel (v1 built vs. production vision)

**What v1 actually captures — real, honest, scoped:** explicit thumbs-up/thumbs-down feedback on individual recommendations, logged with session and category/priority context (Eval Plan Section 3.1). This is the only *truthful* outcome signal available without a real checkout system — the product does not and cannot know whether a recommendation led to a purchase.

**What this deliberately does NOT claim:** purchase confirmation, conversion tracking, or any inference about real-world buying behavior. Section 13 already states there is no checkout flow; fabricating a "did they buy" signal on top of that would violate the same anti-fabrication principle that governs every other part of this product (Rule 6/7, `verify_grounding`). A thumbs-up is a stated opinion at a moment in time — useful, but not evidence of a sale.

**The production vision this hints at (not built, not claimed as built):** in a real Amazon deployment, this same feedback pattern — explicit ratings tied to the specific query context that produced a recommendation — combined with *real* purchase-completion data (available internally to Amazon, not to this prototype) would let a recommendation system learn which specs/price points actually convert for which stated priorities, not just which ones get a thumbs-up. At scale, aggregated across enough sessions, this is the kind of signal that could inform demand forecasting and catalog ranking beyond just this one assistant — closing the loop from "user described a need" to "here's what the market actually wants," rather than relying solely on hand-tuned weights (Section 10) or a static benchmark-derived catalog. This is stated here as a product vision worth articulating, not as a claim about this prototype's current capability.
